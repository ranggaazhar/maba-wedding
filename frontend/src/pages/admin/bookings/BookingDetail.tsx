...
import type { BookingModel, BookingProperty } from '@/types/booking.types';
import { useBookingDetail, paymentStatusConfig } from '@/hooks/Admin/bookings/useBookingDetail';
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-600">Customer telah mengupload bukti transfer. Silakan verifikasi dan konfirmasi atau tolak pembayaran ini.</p>
                  </div>
                </div>
                <div className="flex gap-2 shrink-0">
                  <Button variant="outline" className="border-red-300 text-red-600 hover:bg-red-50" onClick={handleRejectPayment} disabled={isRejecting || isConfirming}>
                    {isRejecting ? <Loader2 size={16} className="mr-2 animate-spin" /> : <XCircle size={16} className="mr-2" />}
                    Tolak
                  </Button>
                  <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={handleConfirmPayment} disabled={isConfirming || isRejecting}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="relative aspect-video rounded-lg overflow-hidden bg-muted">
                      <img src={booking.payment_proof_full_url || booking.payment_proof_url} alt="Bukti Pembayaran" className="w-full h-full object-cover" />
                    </div>
                    <Button variant="outline" className="w-full" onClick={() => window.open(booking.payment_proof_full_url || booking.payment_proof_url, '_blank')}>
                    ...
                    {showConfirmActions && (
                      <div className="pt-3 border-t flex gap-2">
                        <Button variant="outline" className="flex-1 border-red-300 text-red-600 hover:bg-red-50" onClick={handleRejectPayment} disabled={isRejecting || isConfirming}>
                          <XCircle size={16} className="mr-2" /> Tolak
                        </Button>
                        <Button className="flex-1 bg-green-600 hover:bg-green-700 text-white" onClick={handleConfirmPayment} disabled={isConfirming || isRejecting}>
                    ...
                    booking.models.map((model: BookingModel, index: number) => (
                      <div key={model.id || index}>
                        ...
                        <div className="flex gap-4">
                          {model.display_image && (
                            <img src={model.display_image} alt={model.project_title} className="w-28 h-20 rounded-lg object-cover border border-border" />
                          )}
                          <div className="flex-1 space-y-1">
                            {model.category?.name && <Badge variant="outline" className="text-xs mb-1">{model.category.name}</Badge>}
                            <h4 className="font-semibold text-foreground">{model.project_title}</h4>
                            {model.price && <p className="text-sm text-muted-foreground">Rp {parseFloat(model.price).toLocaleString('id-ID')}</p>}
                            {model.notes && <p className="text-sm text-muted-foreground italic">"{model.notes}"</p>}
                          </div>
                        </div>
                      </div>
                    ))
                  ...
                  {booking.properties && booking.properties.length > 0 ? (
                    <>
                      <div className="space-y-3">
                        {booking.properties.map((prop: BookingProperty, index: number) => (
                          <div key={prop.id || index} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                            {prop.display_image && (
                              <img src={prop.display_image} alt={prop.property_name} className="w-12 h-12 rounded-lg object-cover border border-border" />
                            )}
                            ...
                            <div className="text-right">
                              <p className="text-sm font-semibold text-foreground">Rp {parseFloat(prop.subtotal).toLocaleString('id-ID')}</p>
                              <p className="text-xs text-muted-foreground">@ Rp {parseFloat(prop.price).toLocaleString('id-ID')}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-bold text-primary">Rp {totalPropertyCost.toLocaleString('id-ID')}</span>
                      </div>
                    </>
                  ) : (
          ...
          <Card className="border-primary/20">
            ...
            <CardContent className="pt-4 space-y-3">
              ...
              {booking.total_estimate && (
                <SummaryRow label="Estimasi Total" value={`Rp ${parseFloat(booking.total_estimate).toLocaleString('id-ID')}`} highlight />
              )}
              {booking.dp_amount && (
                <SummaryRow label="DP (10%)" value={`Rp ${parseFloat(booking.dp_amount).toLocaleString('id-ID')}`} />
              )}
              <Separator />
